...
def greengrass_hello_world_run():
    try:
        if not my_platform:
            client.publish(
                topic="hello/world", queueFullPolicy="AllOrException", payload="Hello world! Sent from the Lambda function - Greengrass_HelloWorld_Bd7Ah."
            )
        else:
            client.publish(
                topic="hello/world",
                queueFullPolicy="AllOrException",
                payload="Hello world! Sent from " "the Lambda function - Greengrass_HelloWorld_Bd7Ah running on platform: {}".format(my_platform),
            )
